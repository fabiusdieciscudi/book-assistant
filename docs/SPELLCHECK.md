...
| `--dry-run`     | Parse files but skip the actual spell-checking (useful for testing language tag parsing). |
...
- Language tools are initialised lazily (only when a given language is first encountered) and reused across files for performance.