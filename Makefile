...
.PHONY: venv-init clean test1 test2 venv-init test-voices
...
test-voices: $(TEST_RESULTS)
	PYTHONPATH=src python3 test/TestVoices.py \
		--input            test/test1.txt \
		--output-dir       $(TEST_RESULTS)/voices \
		--voices-config    test/multi-voice.conf \
		--instruct-config  test/instruct.conf \
		--qwen3-clone-config $(CLONED_VOICES)/qwen3-clones.conf \
		$(if $(DEBUG),--debug)
